...
def help():
    ...
    [+] -gk || --genkey                  Use this flag to generate a keypair
...
def check_args():
    if not ("-kf" in sys.argv or "--keyfile" in sys.argv):
...
def main():
    ...
    if not ("-gk" in sys.argv or "--genkey" in sys.argv):
        if check_args() == -1:
    ...
    if "-gk" in sys.argv or "--genkey" in sys.argv:
        algo_cipher = "gen"
        source_string = ''
        key_string = ''
    elif "-e" in sys.argv or "--encode" in sys.argv:
        algo_cipher = "enc"
    ...
    rsa(algo=algo_cipher, outfile=outfile, source_string=source_string, key_string=key_string)
